  ...
  return (
    <div>
      <div className="mb-3 flex items-baseline justify-between">
        <label className="font-mono text-[10px] uppercase tracking-[0.24em] text-fog">
          {label}
        </label>
        <span className="font-display text-xl font-bold tabular-nums text-amber-400">
          {format(value)}
        </span>
      </div>
      <input
...
export default function Estimator() {
  ...
  return (
    <section id="estimator" className="relative scroll-mt-20 overflow-hidden bg-pine-900 text-paper">
      <div className="gridlines-light pointer-events-none absolute inset-0 opacity-50" />
      <div className="pointer-events-none absolute -left-40 top-1/3 h-[520px] w-[520px] rounded-full bg-amber-500/12 blur-[130px]" />
          ...
          <div className="lg:col-span-5">
            ...
            <Lines
              ...
              lines={[
                ...
                <span key="b">
                  before you spend<span className="text-amber-500">.</span>
                </span>,
              ]}
            />
            ...
            <ul className="mt-8 space-y-3">
              {[
                ...
                <li key={t} className="flex items-center gap-3 font-mono text-[11px] uppercase tracking-[0.18em] text-fog">
                  <span className="h-1.5 w-1.5 rotate-45 bg-leaf-400" />
                  {t}
                </li>
              ))}
            </ul>
            <p className="mt-10 max-w-sm border-l-2 border-white/15 pl-4 text-xs leading-relaxed text-fog/80">
          ...
          <div className={`reveal lg:col-span-7 ${inView ? "in" : ""}`}>
            <div className="glass-dark rounded-lg p-6 sm:p-8 lg:p-10">
                  ...
                  {PROFILES.map((p) => (
                    <button
                      ...
                      className={`rounded-full px-4 py-2 font-mono text-[10px] uppercase tracking-[0.14em] transition-all duration-300 ${
                        profile === p.id
                          ? "bg-amber-500 font-semibold text-pine-950"
              ...
              <div className="mt-10 grid grid-cols-2 gap-px overflow-hidden rounded-md border border-white/10 bg-white/10 lg:grid-cols-4">
                {[
                  { label: "System size", value: `${r.kwp.toFixed(1)} kWp`, tone: "text-paper" },
                  { label: "Generation / yr", value: fmtGen(r.genKwh), tone: "text-paper" },
                  { label: "Saving / yr", value: `£${Math.round(r.savings).toLocaleString("en-GB")}`, tone: "text-amber-400" },
                  { label: "CO₂ avoided / yr", value: `${r.co2.toFixed(1)} t`, tone: "text-leaf-400" },
                ].map((o) => (
                  <div key={o.label} className="bg-pine-950/70 px-4 py-5">
                    <p className={`font-display text-lg font-extrabold tabular-nums sm:text-xl ${o.tone}`}>
                      {o.value}
                    </p>
                    <p className="mt-1.5 font-mono text-[9px] uppercase tracking-[0.2em] text-fog">
                      {o.label}
                    </p>
                  </div>
                ))}
              ...
              <div className="mt-8 flex flex-wrap items-end justify-between gap-4">
                <div>
                  ...
                  <p className="font-display mt-1 text-3xl font-extrabold text-amber-400">
                    {r.payback.toFixed(1)}{" "}
                    <span className="text-lg font-bold text-fog">yrs</span>
                  </p>
                </div>
                <div className="w-full max-w-[220px]">
                  <div className="h-1.5 overflow-hidden rounded-full bg-white/10">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-amber-500 to-leaf-400 transition-all duration-700 ease-out"
                      style={{ width: `${Math.min((r.payback / 15) * 100, 100)}%` }}
                    />
                  </div>
                  <p className="mt-2 text-right font-mono text-[9px] uppercase tracking-[0.2em] text-fog/70">
                    vs 25-yr asset life
                  </p>
                </div>
              </div>
